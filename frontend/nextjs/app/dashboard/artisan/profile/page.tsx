    ...
    <DashboardLayout role="artisan">
      <div className="space-y-6 max-w-3xl mx-auto">
        <nav className="mb-2 flex items-center gap-2 text-sm text-muted-foreground">
          <Link href="/dashboard/artisan" className="transition hover:text-foreground">Accueil</Link>
          ...
          <Card className="border-0 shadow-md bg-card">
            ...
            <CardContent className="space-y-6">
              <div className="flex items-center gap-5">
                <div className="relative">
                  <Avatar className="h-20 w-20 ring-2 ring-primary/10">
                    ...
                    />
                    <AvatarFallback>
                      {profile ? `${profile.firstName[0]}${profile.lastName[0]}` : user ? `${user.firstName[0]}${user.lastName[0]}` : "AR"}
                    </AvatarFallback>
                  </Avatar>
                ...
                <div>
                  <h3 className="font-semibold text-foreground">
                    {profile ? `${profile.firstName} ${profile.lastName}` : user ? `${user.firstName} ${user.lastName}` : "Artisan"}
                  </h3>
              ...
              <div className="grid gap-5 sm:grid-cols-2">
                ...
                <div className="space-y-1.5">
                  <Label className="text-xs text-muted-foreground font-bold flex items-center gap-1.5">
                    <User className="h-3.5 w-3.5" /> Région
                  </Label>
                  {isEditing ? (
                    <Select value={formData.region} onValueChange={(value) => handleInputChange("region", value)}>
                      ...
                      <SelectContent>
                        <SelectItem value="Alaotra-Mangoro">Alaotra-Mangoro</SelectItem>
                        <SelectItem value="Amoron'i Mania">Amoron'i Mania</SelectItem>
                        <SelectItem value="Analamanga">Analamanga</SelectItem>
                        <SelectItem value="Analanjirofo">Analanjirofo</SelectItem>
                        <SelectItem value="Androy">Androy</SelectItem>
                        <SelectItem value="Anosy">Anosy</SelectItem>
                        <SelectItem value="Atsimo-Andrefana">Atsimo-Andrefana</SelectItem>
                        <SelectItem value="Atsimo-Atsinanana">Atsimo-Atsinanana</SelectItem>
                        <SelectItem value="Betsiboka">Betsiboka</SelectItem>
                        <SelectItem value="Boeny">Boeny</SelectItem>
                        <SelectItem value="Bongolava">Bongolava</SelectItem>
                        <SelectItem value="Diana">Diana</SelectItem>
                        <SelectItem value="Ihorombe">Ihorombe</SelectItem>
                        <SelectItem value="Itasy">Itasy</SelectItem>
                        <SelectItem value="Melaky">Melaky</SelectItem>
                        <SelectItem value="Menabe">Menabe</SelectItem>
                        ...
                        <SelectItem value="Vakinankaratra">Vakinankaratra</SelectItem>
                        <SelectItem value="Vatovavy">Vatovavy</SelectItem>
                        <SelectItem value="Fitovinany">Fitovinany</SelectItem>
                      </SelectContent>
                    </Select>
                  ) : (
                    <div className="flex items-center rounded-md bg-muted/20 px-3 py-2 min-h-9">
                      <p className="text-sm text-foreground">{profile?.region || "Non définie"}</p>
                    </div>
                  )}
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="flex items-center rounded-md bg-muted/20 px-3 py-2 min-h-9">
                    <p className="text-sm text-foreground">Artisan</p>
                  </div>
                </div>
                <div className="space-y-1.5">
                  <Label className="text-xs text-muted-foreground font-bold flex items-center gap-1.5">
              ...
              <div className="space-y-1.5">
                <Label className="text-xs text-muted-foreground font-bold">Bio / Description</Label>
                {isEditing ? (
                  <textarea
                    id="bio"
                    value={formData.bio}
                    onChange={(e) => handleInputChange("bio", e.target.value)}
                    placeholder="Décrivez-vous et votre travail..."
                    className="w-full min-h-[80px] rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2"
                    rows={3}
                  />
                ) : (
                  <div className="rounded-md bg-muted/20 px-3 py-2">
                    <p className="text-sm text-foreground">{profile?.description || "Non définie"}</p>
                  </div>
                )}
              </div>
